...
#}
{% macro prune_keys_not_in_bronze(bronze_table, silver_key, bronze_key='id') %}
delete from {{ this }} as s
where not exists (
    select 1
    from {{ source('src_local_postgres', bronze_table) }} as b
    where cast(b.{{ bronze_key }} as bigint) = s.{{ silver_key }}
)
{% endmacro %}
